...
## 2026-08-30 — 오버레이 재설계 2차: 카드 제거, Pretendard, 런타임 통계 분리

### 먼저: 직전 커밋(12b4f3b)이 폰트를 통째로 날려먹고 있었다

`widgets.cpp`의 폰트 경로를 `"C:\\Windows\\Fonts\\malgun.ttf"`가 아니라 역슬래시 한 번짜리
`"C:\Windows\Fonts\malgun.ttf"`로 써 놓았다. MSVC는 이걸
에러가 아니라 **경고 C4129**로 넘기면서 `\W`/`\F`/`\m`을 각각 `W`/`F`/`m`으로 접어 버리므로,
런타임 문자열은 `C:WindowsFontsmalgun.ttf`가 된다. `GetFileAttributesW`가 당연히 실패하고
`AddFontFromFileTTF`는 아예 호출되지 않아서, 오버레이 전체가 ImGui 기본 폰트(ProggyClean 비트맵)로
그려졌다. 사용자가 본 증상 두 가지가 여기서 한꺼번에 나온다:

- UI 전체가 픽셀/모노스페이스 폰트로 보임 (ProggyClean이 그렇게 생겼다)
- 한글이 전부 `?`로 깨짐 (ProggyClean에 한글 글리프가 없다)

즉 "디자인이 별로다"의 상당 부분이 실은 폰트가 한 줄도 로드되지 않은 상태였다. 빌드 로그에 C4129가
25줄 찍혀 있었는데 직전 세션에서 그걸 읽고 넘어갔다. **경고를 흘려보내지 말 것** — 이 프로젝트에서
문자열 리터럴의 경로는 전부 `\\`로 쓴다.

### 폰트: Pretendard → 맑은 고딕 → 기본 폰트

`tools/scripts/fetch_fonts.py`를 추가했다. Pretendard 1.3.9 릴리스 zip에서 Regular/SemiBold만 꺼내
`%LOCALAPPDATA%\cbpk\fonts\`(config.ini와 같은 뿌리)에 넣는다. 저장소에 폰트를 커밋하지 않은 이유는
OFL 라이선스 + 웨이트당 2.7 MB라 텍스트 트리에 들어갈 물건이 아니어서다. 없으면 Windows에 항상 있는
맑은 고딕으로 조용히 내려간다 — 한글이 깨지지 않는 것이 조판보다 우선이라, 한글 글리프가 없는
Segoe UI는 본문 후보에서 뺐다. 통계 숫자에만 Consolas 계열을 따로 얹는다.

### 설계 변경 (사용자 피드백 반영)

문제 진단: 요소가 많아서가 아니라 **전부 같은 시각적 무게**를 갖고 있었다. 카드 안에 카드, 어디에나
테두리, 작은 글자, 라벨에서 토글까지의 긴 빈 공간.

- **카드 전면 제거**. 설정 8개짜리 화면에 둥근 박스가 세 겹씩 겹치면 UI가 실제 정보량의 서너 배로
  무거워 보인다. 이제 섹션 제목 + 얇은 규칙선 + 여백만 남았다.
- **회색 단계 7 → 4** (window / sidebar / surface / hover). 예전 팔레트는 면이 일곱 개였고 절반이
  서로 3% 안에 모여 있어서 구역은 구분되지 않고 화면만 탁했다.
- **본문 폭 660px 상한**. 넓은 창에서 라벨과 토글 사이가 600px 넘게 벌어져서 어느 스위치가 어느
  라벨의 것인지 눈으로 잇기 어려웠다.
- **사이드바 198 → 164px**, 선택 표시를 진한 파란 블록에서 accent 10% 배경 + 2px 좌측 바로 축소.
  화면에서 가장 강한 색이 "지금 어느 탭인지"라는 가장 안 중요한 정보에 붙어 있었다.
- **Language/Theme를 사이드바에서 창 하단 푸터로**. 내비게이션이 아닌데 Aimbot/ESP/Misc/Debug와
  같은 계층에 있어서 섹션이 여섯 개인 것처럼 보였다. 푸터 오른쪽 끝에는 단축키 안내를 둔다
  (버전 문자열은 코드베이스에 아직 없어서 넣지 않았다).
- **Debug 페이지 재계층화**: Overlay / 개발자 진단 / 실험적 기능. `Show FPS`와 `Crash reporting`은
  중요도가 완전히 다른데 같은 목록에 있었다. 실험적 기능에는 섹션 설명 한 줄과 항목별 부연을 붙인다.
- **내부 통계 → 접히는 `런타임 통계`**. 예전에는 마지막 토글 아래에 원시 텍스트가 그대로 이어져서
  설정의 연장처럼 보였다. 이제 그룹 제목 + 상태 배지 + "값 이름" 쌍을 폭에 맞춰 흘려 넣고,
  숫자만 고정폭으로 찍는다. `Entity feed: hooked | registered 68 | positioned 68 | ...` 한 줄을
  파이프로 이어 붙이면 어디가 값이고 어디가 이름인지 매번 다시 읽어야 한다.

### 검증 상태

Release 클린 빌드 통과, 경고 0 (C4129 포함 전부 사라짐). **인게임 확인은 아직 못 했다** —
확인하려던 시점에 게임(pid 29968)이 19:22:06부터 멎어 있었다: `Responding=False`, 트레이너 로그도
그 시각 이후 없음, CPU는 계속 도는 중. 메인 틱이 안 돌아서 `inject.py --unload`의 ack가 오지 않고,
그래서 `build/bin/Release/cp2077_trainer.dll`이 잠긴 채라 그 트리로는 링크조차 못 한다(LNK1104).
...
## 2026-09-06 - Visibility world-clear safety

- Visibility now checks EntityTracker's shared world readiness before metadata resolution/system acquisition. Closed or empty-work ticks do no Visibility engine work. Query returns Unknown while closed and cannot refill the queue during transition.
- Under the Visibility SRW lock, each observed gate transition increments a local world generation and clears queue, cache/pending flags and cached raycast metadata. Reopening waits for the existing tracker settle gate; only fresh requests bootstrap resolution. Resolver failures remain retryable. Requests/cache carry the generation; primary/secondary processing and result publication validate it. No engine call holds the queue lock.
- Dump markers in visibility.cpp: g_mainTickStage (0 Idle, 1 WorldGate, 2 ResolveMetadata, 3 AcquireSystem, 4 PrimaryCast, 5 SecondaryCast, 6 Publish), g_mainTickGeneration, g_mainTickEntityId. Stage returns to Idle before original OnTick. g_worldGeneration is a Visibility gate epoch, not an engine world ID. Gate logs include ready/generation.
...
## 2026-09-07 - Skeleton limbs, multiple aim bones, dual activation keys and two profiles

- Screenshot issue reproduced in source: the old pose reader collected only Head/Chest/Hips/RightHand/LegLeft/LegRight, producing five segments. Left arm, shoulder/elbow joints and lower legs were absent by construction.
- Cyberpunk 2077 2.31 live read-only inspection of `entSlotComponent.slots` (SDK offset +0x120, entSlot stride 0x30) found LeftArm/RightArm, LeftForeArm/RightForeArm, LeftHand/RightHand, LeftUpLeg/RightUpLeg and LeftFoot/RightFoot. No new game-memory writes or native function hooks were introduced.
- Capture now publishes 16 named landmarks and 15 segments. Live sample at 17:41:57 (PID 20628) verified all limb slots. Slot names are misleading: LegLeft Z=-214.488 was the knee, LeftUpLeg Z=-214.981 was the ankle, LeftFoot Z=-215.032 was the foot endpoint, with Hips Z=-214.049. Final edges use hips -> knee -> ankle -> foot, with both shoulders -> elbows -> hands.
- Neck uses the authored Neck slot when available; otherwise it is a 65% chest-to-head interpolation, not a verified neck bone transform. Missing limb slots are skipped rather than invented. Pose work stays on the retained-entity game main tick with no own lock during VM calls. Per-tick entity budget reduced 24 -> 8 to keep the worst-case slot-call budget below the previous 24 x 6 capture. Diagnostic validity mask expanded to 32 bits.
- Aimbot supports Head/Neck/Chest/Arm/Leg multiple selection, with closest-to-crosshair selection within checked groups. Nearest searches all available groups. Unavailable selected limbs do not fall back to head. Lead velocity is now estimated from entity root motion so a change of selected bone does not create false target velocity.
- Main and optional Sub activation keys use OR semantics. Two complete aim profiles retain their independent settings. Menu selection or the configurable switch key (default F6) swaps the active profile; the hotkey is edge-triggered, game-foreground-only, and suppressed during menu/key capture. Switching clears classic lock, silent target grace and armed orientation state. A short HUD message identifies the newly active profile.
- INI preserves legacy active settings and seeds an absent alternate profile from them. Both profiles, active index, switch key, bone masks and Sub key persist; previously omitted lead_prediction also persists and participates in dirty detection. ESP category-name and distance labels now have independent toggles.
- UI uses existing custom draw widgets. Verified Korean/light aimbot and English/dark ESP with the D3D12 preview harness; bone controls fit the right column without overlap. Screenshot mode now hides the preview window. Artifacts: tools/scripts/temp/aimbot_updated.bmp and esp_updated.bmp (untracked).
- Validation: full Release build succeeded; `cmake -S . -B build -DCP2077_BUILD_TESTS=ON` and `ctest --test-dir build -C Release --output-on-failure` passed config_profiles. Tests exercise legacy migration, independent full-profile save/reload after switching, disabled Sub key, label toggles, debounce autosave and malformed mask/key/profile input. `git diff --check` passed.
- Live validation: used inject.py safe unload/reinject (PID 20628). Intermediate live capture logged 165 skeleton segments for 11 drawn NPCs, all 15 per NPC; this exposed the ankle slot naming and informed the final edge-order correction. Final DLL reinjected at 17:47 and main tick / overlay submission continued with allocatorMisses=0. Final session has not yet streamed fresh NPC registrations, so final edge order is verified against captured coordinates, not a new visual combat sample.
- Remaining optional extension: per-shotgun-pellet bone distribution is NOT implemented or exposed. Current hitscan mutation is a shared crosshair-direction hook with no verified pellet ID/batch boundary; a per-call round-robin would not establish per-pellet control. Shotgun-equipped live testing was requested asynchronously but no response or matching firing sample was received during this work. Multi-bone selection itself is implemented for classic and silent modes.

## 2026-09-07 - Bounded shot/crosshair correlation instrumentation (F8)

- User authorized instrumentation to distinguish weapon firing from shared crosshair updates before attempting per-pellet silent-aim distribution. This change adds observation only; no new mutation hook, distribution logic, or time-burst shotgun classifier.
- Added `src/game/shot_trace.{h,cpp}`. With diagnostic logging enabled, press F8 while the game is foreground and the menu is closed to start a 20-second capture; F8 again stops it. Menu opening or loss of game foreground also stops capture. A REC overlay is visible during capture. Capture is OFF after injection.
- Hot callbacks copy POD records into a fixed 4096-cell nonblocking multi-producer buffer. A producer makes one cell claim attempt and drops on contention/full buffer; no waiting, allocation, formatting, disk I/O, Script VM call, or new engine call in the trace producer. Capture has a 24000-record budget and reports cumulative loss at STOP. Original diagnostic writer drains and formats batches directly to the trainer log, avoiding the existing 512-line log queue. No additional thread or unload join was added.
- `[SHOTTRACE]` kinds: 0 START (id=QPC frequency, object=game base, context=GetTickCount64, event=prior loss count), 1 STOP (id=cumulative loss, flags=submission attempts), 2 input/settings edges, 3 weapon metadata, 4 crosshair invocation, 5 queued event. Every record includes capture ID, sequence, QPC, producer thread, caller return address and raw identity fields. Writer order is not temporal order; sort by capture/QPC/sequence.
- Crosshair records preserve entry and exit QPC, immediate caller, system/query/filter identities, origin, original returned direction before existing redirection, final direction and target generation. flags: 1 useRaycast, 2 redirected, 4 valid origin/before reads, 8 valid after read. Added vector reads are SEH guarded and never mutate arguments.
- Queue instrumentation records all observed existing weapon ShootEvent/spawner/Shoot/ShootTarget dispatches before the previous processing. Event/receiver pointers and runtime class hash/event ID are logged. These are dispatch observations, not unique shots: duplicate deliveries and NPC events remain possible. Unsafe weapon-listener hooks and producer VM observation hooks remain disabled.
- Weapon metadata samples run only on game main tick while capture is active, at most every 250 ms. Reuse the existing retained player/right-hand weapon lookup; log the weapon pointer only as a numeric identity, never dereference it after release. Query `gamedataStatType::ProjectilesPerShot` (1167 in the local RED4ext SDK for 2.31, Generated/game/data/StatType.hpp). Query failure remains -1. The sample is metadata, not proof of shotgun type or a pellet boundary.
- Input record low bits: LMB=1/RMB=2; bits 8/9 aim-enabled/silent, bits 10/11 no-spread/no-recoil, bits 12..16 selected bone mask, bit 17 nearest, bit 18 active profile. Input edges are frame-sampled, not native firing confirmations.
- Added `tools/scripts/shot_trace_report.py`: default reads `%LOCALAPPDATA%/cp2077_trainer/cp2077_trainer.log`; optional positional path honors custom log locations, `--json <path>` exports detailed capture reports. Summarizes caller/thread counts, gaps, redirects, input edges, weapon samples, queue receiver identity hints, missing STOP and reported drops. Never labels close calls as pellets. Handles ring order and capture number reuse across reinjection.
- Validation: full Release DLL build successful; CTest config_profiles and shot_trace passed. Shot trace harness exercises 8 concurrent producers / 32000 submissions, bounded budget/drop accounting, record integrity, unique sequence IDs and rejection of stale cross-capture callbacks. Python report tests cover out-of-order records, reinjection, incomplete captures and flag decoding; both passed. `git diff --check` passed.
- Safe unload/reinjection succeeded in PID 20628. At 19:38:40.899 the live log confirms `shot trace ready: F8=20s capture/stop ... mutation=0`; logging=1. Render submissions continued with allocatorMisses=0. No live F8 capture has been received yet, so weapon-stat values and event-to-crosshair relationships still require the user's shotgun sample.
- Test procedure: close menu, F8, aim without firing for ~2 seconds, fire 3-5 separated shotgun shots (~1 second apart), wait for automatic stop or press F8. An optional second capture with a single-projectile weapon provides a control sample. Analyze only after checking capture completeness/loss; absence of a queue event is not proof that no shot occurred.

## 2026-09-07 - Final shotgun trace analyzed (capture 3 only)

- Operator excluded earlier accidental captures. Analyzed only capture 3, START QPC 375510320520, 19.994 seconds, START/STOP present, drops=0; all 1204 sequence numbers 90..1293 unique/contiguous.
- 80 weapon samples consistently reported ProjectilesPerShot=6; silent/head/profile1 active, no-spread/no-recoil off.
- 1090 crosshair calls split into caller RVAs 0x2A60F85 (999), 0x2048BED (42), 0x657410 (49). Seven groups at 0x2048BED each contain six distinct original directions changed into one identical final direction. This is a strong per-projectile-path candidate, not yet a verified pellet loop. Matching seven-call groups at 0x657410 remain unexplained; zero queue events were observed.
- Findings and group timings saved in reports/2026-09-07_shot_trace_capture3.md. No code/build/injection changes. Next step is caller/loop disassembly, not time-burst classification. Documentation validation: git diff --check.

## 2026-09-07 - Explicit pellet loop and multi-region distribution

- Operator confirmed capture-3 weapon DB-4 Igla has six projectiles. Installed 2.31 executable disassembly identified explicit byte-count loop at RVA 0x659C5C, count second-argument+0x1C0, one call to 0x2048B7C per iteration; counter R15B increments before count comparison. This establishes the first-pass pellet boundary independently of timing.
- Core generates a spread quaternion into its fifth argument; aim-param+0x80 is copied intact and reused by wrapper 0x6573A8 on later worker threads. Implemented first-pass assignment plus bounded exact-quaternion replay cache, preserving shot/index/target across threads. Full code checksums of three functions gate the version-specific hook; local equipped-weapon identity gates shot scopes. No additional VM/engine calls or locks across engine calls.
- Present publishes one eligible closest joint per selected group on the selected entity. Two or more regions cycle per shot (Head+Chest => 3/3 for six pellets). Missing/out-of-FOV groups are omitted. Nearest, single-region and no-spread use ordinary closest-bone aim; identical no-spread rotations cannot carry distinct pellet identity. Duplicate keys, stale epochs and cache contention fail back to ordinary targeting.
- F8 kinds 6..9 now expose native shot boundaries, expected/observed counts, first-pass bone assignment and exact replay indices. Report parser summarizes these IDs rather than inferring batches from timestamps. Detailed offsets, cache semantics, trace format and limits: reports/2026-09-07_pellet_distribution.md.
- Release build and all three CTests passed; three Python report tests passed. New tests cover balanced assignment, exact identity, expiry, invalidation, ambiguous rotations and concurrent cache access. Korean/light UI preview inspected; helper text fits. Safe unload/reinjection PID 20628 succeeded; new hook confirmed at 20:20:39.704 and subsequent main ticks/ESP remained active.
- Live pellet replay and actual hit distribution still need a fresh Head+Chest F8 firing sample. User was asked asynchronously for 2–3 Igla shots with Nearest/no-spread off. Do not claim those outcomes from the old head-only capture.
- Fresh live sample arrived before completing this milestone: latest capture 2, QPC 386222360463, 15.861 seconds, 562 unique/contiguous records (seq 1002..1563), START/STOP present, drops=0. Shots 5..11 each observed exactly six calls and assigned Head/Chest/Head/Chest/Head/Chest. All 42 downstream replays recovered the matching index, bone and exact frozen target; seven unrelated extra queries remained unassigned. This verifies the implemented cross-thread direction distribution. Damage/collision bone IDs are not recorded and remain outside this evidence. Final capture extraction and machine report are in untracked tools/scripts/temp/pellet_final_capture.log and pellet_trace_report.json.

## 2026-09-07 - Remove extra pose sampling delay

- User observed animation lag in skeletons. Source confirms ESP and classic/silent aim share cached visual joints; the pose scheduler imposed a 33 ms minimum interval in addition to an eight-entity round-robin budget. Recent limb expansion reduced the old entity budget from 24 to 8.
- Removed the wall-clock interval so requested entities can be sampled every main tick. Preserved the eight-entity cap, request expiry, retained handles, complete-sample publication and main-tick-only VM calls. Corrected stale activation diagnostics from 24 entities to the actual 8.
- This increases average sampling work when fewer entities are requested; maximum entities per tick stays unchanged. Crowded scenes still require multiple ticks, and engine animation update ordering remains unmeasured. No claim of zero latency or measured visual improvement.
- Release build and git diff --check passed. Safe unload/reinjection succeeded in PID 20628; subsequent overlay cadence reported 534 submissions and allocatorMisses=0. Visual animation-following improvement requires an in-game comparison.

## 2026-09-07 - Consumer-filtered pose demand and measured freshness

- User requested measurement-driven diagnosis of civilian/unclassified pose work and excessive throttling. Confirmed GetPuppetSnapshots requested all 128 returned NPCs before ESP/aim filters. Live baseline: 591 ticks/5s, 4728 entity pose reads, mean pose pass 124.1 us, per-entity slots 11.5 us; only three ESP categories enabled among 128 snapshots. Thus the previous 33 ms-only fix missed irrelevant requests in the eight-entity queue.
- Features now publishes explicit ESP/aim union ID requests after shared category/hostility, death, visual-output and aim health filters. Headless excludes ESP, native-only ESP needs no pose. Conservative root depth gate uses consumer distance limits plus a three-metre margin, with projection failure allowing recovery. No FOV/occlusion gate that depends on stale joints. Disabled requests cancel immediately; reactivation clears the old cache until a new sample arrives. Snapshot capacity now matches the 256-entry tracker.
- Added profiler pose request-pass cost (included in Present graph), requested/eligible/processed/deferred counts, per-NPC publication interval, drawn skeleton cache age, selected aim-candidate cache age and requested archetype counts. Existing pose-pass and per-entity slot timing remain. Values and limitations documented in reports/2026-09-07_pose_scheduling.md.
- Filtered active sample at 20:40:51: four requests, four processed every tick, zero deferred; mean publication interval 10.6 ms, drawn skeleton age 3.9 ms. Kept eight-entity cap because it did not limit this measured sample; no claim that eight is universally sufficient. Initial candidate-wide aim-age metrics included rejected targets and are excluded from active-aim conclusions; final metrics use the selected candidate.
- Final-build negative-demand sample at 20:43:37: 20 snapshots (one civilian, 19 other, none hostile), zero category-enabled entries, zero pose requests/processing and all requested archetype counts zero. Final-build active-aim freshness still needs a corresponding live target sample. These are different streamed scenes, not a controlled total-FPS benchmark; engine animation transform cadence is not measured by cache freshness.
- Release build passed; four CTests passed, including new target-filter regression cases. git diff --check passed. Safe unload/reinjection succeeded in PID 20628. No additional VM hooks or synchronous disk work added.

## 2026-09-07 - Transparent hierarchical performance HUD

- User requested a fully background-free performance graph and detailed time breakdown by hierarchy. Replaced panel/plot fills, outer borders and panel shadows with transparent line charts and shadowed text. Standalone FPS is also text-only. Existing graph opacity setting now controls text/line ink; renamed Korean/English setting labels and retained INI compatibility.
- Split Present feature CPU and main-tick trainer CPU into separate colored curves; stopped adding unrelated thread samples. Frame-time chart includes a faint per-bin maximum alongside mean. History uses 100 ms bins (approximately 12 seconds, extended by long frames), with no EMA that hides spikes. CPU curves average last-observed samples at Present cadence, not an exact CPU utilization measurement.
- Added 19-row timing hierarchy with collapsible Present/main-tick branches. Includes snapshot/lock wait, pose request pass, ESP, aim, pose/joint slots, health collect/invoke, attitude collect/invoke, native highlight collect, player modifiers and visibility. Columns show inclusive mean microseconds per corresponding Present frame/main tick, maximum per invocation, and calls per frame/tick. Nested rows must not be summed; GPU and unmeasured hook costs are excluded and labeled.
- Added pose delivery rows for requested/processed/deferred NPCs and update gap/ESP age/selected aim age (mean/max). Preserved Insert-menu-only header dragging, saved position and click-through gameplay. Click branch headings with menu open to collapse; short displays allow wheel scrolling in menu. No-data/disabled profiling renders dashes and explicit waiting/off state.
- Profiler publishes a nonblocking read-only snapshot of the same completed five-second windows used by logs. Record producers keep their existing atomics; no extra per-scope lock, game call, VM call or disk I/O. New PresentTotal is the sum of nonoverlapping feature scopes, independently tested against nested lock timing and main-tick timing. UI reads do not drain counters. One short try-lock copies each published window; failed acquisition skips publication/read rather than waiting.
- Preview harness now supports --performance, with clearly synthetic data confined to the preview target. Inspected 960x720 full hierarchy and light-theme profiling-off renders; no text overlap/clipping, backdrop remains visible and no panel rectangles. Screenshots (untracked): tools/scripts/temp/performance_transparent.bmp and performance_off.bmp.
- Release build passed. All five CTests passed, including profiling_window (unit conversion, nested-scope exclusion, counter units, nondestructive UI reads, absent metrics and reset/re-enable behavior). git diff --check passed. Safe unload/reinjection succeeded in PID 20628; live logs at 20:57:42 confirm presentFeatures window and continuing overlay submissions with allocatorMisses=0. Synthetic preview values are not live performance results.

## 2026-09-07 - Performance HUD background, readable type and Simple/Advanced modes

- User revised the transparent HUD request: add a subtle background, minimum 12 px text and selectable Simple/Advanced views. Added a rounded dark translucent background (45% default, independently configurable 0-85%). Text/line opacity remains independent and existing INI keys retain their meaning. No true backdrop blur was added: the renderer has no blur pass; this implementation uses alpha blending without new GPU resources or backbuffer copies. Communicated this limit explicitly.
- Enforced a HUD-local 12 px minimum for every label/legend/footer and 14 px numeric text using the existing font faces; right-alignment measures at the same actual draw size. Expanded rows to 24 px and panel width to 662 px. Advanced view scrolls on short displays rather than shrinking text below the minimum.
- Added persisted graph_advanced (default false) and graph_background_opacity. Simple mode is a 300 px-high panel containing FPS/frame-time trend, separate Present/main-tick CPU curves and CPU mean/max summaries. Advanced retains the timing hierarchy, collapse controls and pose delivery metrics. Mode tabs on the HUD are interactive only with Insert menu open; Debug settings also expose the mode and background slider. Both settings participate in dirty detection/autosave.
- Preview harness --variant=advanced selects the expanded view. Inspected simple (960x720), advanced (1000x900) and Korean Debug settings (960x780); no overlapping text or controls in these layouts. Untracked artifacts: tools/scripts/temp/performance_simple.bmp, performance_advanced.bmp and performance_settings.bmp.
- Release build passed and all five CTests passed. Extended config regression covers legacy defaults, mode/background round-trip, dirty autosave and background bounds. git diff --check passed. Safe unload/reinjection succeeded in PID 20628.
